class Snake{
	...
	drawBody(x, y, i) {
    // Pulse effect based on time and segment index
    const pulse = 0.5 + 0.3 * Math.sin((Date.now() / 200) - i * 0.5);
    ...
    // Radial gradient body core
    const grd = this.ctx.createRadialGradient(x, y, 2, x + 4, y + 4, 10);
    grd.addColorStop(0, this.supportColor);
    grd.addColorStop(1, this.midColor);

    let radius = this.size - (i * 0.01);
